...
0. Cours sur Packer
1. Créez une nouvelle issue nommée `Création d'une image VM Scaleway avec Packer` puis créez sa Merge Request. Ensuite pullez le code, et changez de branche pour utiliser la nouvelle branche (sur le project image).
2. Nous allons utiliser l'image `ubuntu_focal`, le commercial_type `DEV1-S`, le ssh_username `root`
...
5. Complétez votre `before_script` suivant à votre fichier `.gitlab-ci.yml` avec (pensez à remplacer le numéro de groupe):
   ...
   before_script:
     - |
       ...
       export SCW_TOKEN="$(vault kv get -field=SCW_TOKEN secret/groupe-<group_number>/scaleway)"
     ```
6. Demandez une revue de code à votre professeur, puis une fois la Merge Request approuvée, mergez la branche puis taguez la branche master en `1.0.1`
